...
class karyawanController extends Controller
{
    ...
    public function index(Request $request)
    {
        if ($request->has("search")) {
            ...
            ->join('karyawan', 'users.karyawan_id', '=', 'karyawan.id_karyawan')
            ->where(function($query) use ($request) {
                      $query->Where('karyawan.nama', 'like', "%".$request->search."%")
                      ->orWhere('karyawan.alamat', 'like', "%".$request->search."%")
                      ->orWhere('karyawan.jenis_kelamin', 'like', "%".$request->search."%");
            })
            ->paginate(15);
        }else {
            $karyawan = user::paginate(15);
        }
